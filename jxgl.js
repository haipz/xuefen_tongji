...
function dealWithDiffType(strType) {
	...
	for (var i = 0; i < total; i++) {
		...
		} else if (strType == "其他限选") {
			if ((courseType.indexOf(strType) >= 0 || courseNature.indexOf(strType) >= 0) && (courseType.indexOf("专业限选") < 0 && courseNature.indexOf("专业限选") < 0)) {
				sum += parseFloat(obj.eq(6).text())
				saveIntoLocalStorage(obj.eq(0).text() + "-" + obj.eq(1).text() + " " + obj.eq(3).text() + " " + courseNature + " " + courseType + " " + obj.eq(6).text())
			}
		} else {
...
function getAll() {
	...
	for (var idx in sumList) {
		var info=sumList[idx]
		checkSum+=parseFloat(info.substring(info.lastIndexOf("：")+1))
